...
function lib:GetValue(item)
  ...
  --DKPmon:Print("updated Recent Loot")
  ...
  --DKPmon:Print("Returning Results "..high)  
...
local PRE_WOTLK_ILVL_TO_IVALUE = {
...
local PRE_WOTLK_EQUIPSLOT_VALUE =  {
  ["INVTYPE_HEAD"] = 1,
  ["INVTYPE_NECK"] = 0.55,
...
--Used to display GP values  directly on  tier tokens
local PRE_WOTLK_CUSTOM_ITEM_DATA = {
  --CUSTOM COST  ITEMS
...
function lib:GetGPValuePreWotlk(itemLink)
  local retVal = 0;
  ...
  if (itemLink ~=  nil) then 
    ...
    if CUSTOM_ITEM_DATA[ctrt_itemID]  then
      ctrt_rarity, ctrt_level, ctrt_equipLoc = unpack(PRE_WOTLK_CUSTOM_ITEM_DATA[ctrt_itemID]);
    end

    CT_RaidTracker_Debug("Post:", "CUSTOM_ITEM_DATA unpack", "ctrt_equipLoc:", ctrt_equipLoc);
    
    local ctrt_islot_mod = PRE_WOTLK_EQUIPSLOT_VALUE[ctrt_equipLoc];
    ...
    if (ctrt_islot_mod ~= nil) then
      if (ctrt_islot_mod ~= -1) then 
        local ctrt_ilvl2ivalue = PRE_WOTLK_ILVL_TO_IVALUE[ctrt_rarity]; --get the item level value function
